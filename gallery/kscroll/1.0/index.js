...
KISSY.add("gallery/kscroll/1.0/index",function(S, Node){
    ...
    function Scroll(container, config) {
        Scroll.superclass.constructor.call(this, config);
	...
	S.extend(Scroll, S.Base, {
        ...
        _init:function(container,config){
            ...
            //设置属性
            cfg.prefix && this.set("prefix",cfg.prefix);
            cfg.arrow && this.set("arrow",cfg.arrow);
        ...
        _wrap: function(container){
			...
			self.set("cachestyle",panel.attr("style"));	
        ...
        _bindEvt: function(){
            ...
            this.get("container").on("mousewheel", function(ev){
                ...
                self.scrollDistance(ev.deltaY>0?sh:-sh);
            });
					...
					.on("mousedown", function(ev){
						...
						var tg = $(ev.target),
							mouseup = function(){
								...
								$(document).detach("mouseup",mouseup);
							};
		...
		destory: function(){
			var self = this;
			self.get("body").attr("style",self.get("cachestyle")).insertBefore(self.get("container"));
